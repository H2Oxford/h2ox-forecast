...
def download_tigge(today, timedelta_days, email, key):
    
    server = ECMWFDataServer(email=email, key=key)
    
    end_dt = today + timedelta(days=timedelta_days)
    
    fname = os.path.join(os.getcwd(),f'{today.isoformat()[0:10]}_{end_dt.isoformat()[0:10]}.grib')
...
def ingest_local_grib(grib_path, gcs_root, n_workers, zero_dt):
    ...
    ds_dask =  xr.open_dataset(grib_path, chunks = {'longitude':10,'latitude':10,'steps':61,'time':1461}, engine='cfgrib')
